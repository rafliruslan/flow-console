...
struct Archive {
  ...
  private init() throws {
    ...
    try self.tmpDirectoryURL = fm.url(for: .itemReplacementDirectory,
                                      in: .userDomainMask,
                                      appropriateFor: FlowConsolePaths.blinkURL(),
                                      create: true)
  }
  ...
  static func recover(from archiveURL: URL, password: String) throws {
    // Extract
    let homeURL = URL(fileURLWithPath: FlowConsolePaths.homePath())
  ...
  private func copyAllDataToTmpDirectory() throws {
    ...
    // Copy everything
    let blinkURL = FlowConsolePaths.blinkURL()!
    let sshURL   = FlowConsolePaths.sshURL()!
