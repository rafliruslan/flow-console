...
extension Bundle {
  func receiptB64() -> String? {
    // For free/open source version, return a dummy receipt for Build service compatibility
...
enum BuildAPI {
  ...
  private static func _baseURL() -> String {
    if FileManager.default.fileExists(atPath: FlowConsolePaths.blinkBuildStagingMarkURL()!.path) {
  ...
  static func signup(email: String, region: BuildRegion) async throws {
    ...
    let (code, data, _) = try await _post(
      ...
      params: [
        ...
        "rev_cat_user_id": "unlimited_user",
  ...
  static func loginWithToken(token: Data) async throws {
    try token.write(to: FlowConsolePaths.blinkBuildTokenURL()!)
